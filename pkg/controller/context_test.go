...
func TestSyncContext_eventHandler(t *testing.T) {
	tests := []struct {
		...
		// func (c Context) EventHandler(queueKeyFunc ObjectQueueKeyFunc, interestingNamespaces sets.String) cache.ResourceEventHandler {

	}{
		{
			name:        "simple event handler",
			syncContext: controllercontext.New("test", eventstesting.NewTestingEventRecorder(t)),
		...
		{
			name:        "event handler returning multiple keys",
			syncContext: controllercontext.New("test", eventstesting.NewTestingEventRecorder(t)),
			queueKeyFunc: func(object runtime.Object) []string {
				m, _ := meta.Accessor(object)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			handler := test.syncContext.(controllercontext.Context).EventHandler(test.queueKeyFunc, test.filterFunc)
